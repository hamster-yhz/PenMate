...
(921001, 921001, 'dbcase_boundary_user1@penmate.local', '$2a$10$boundary', RPAD('U', 80, 'U'), 0, 'local', NULL, NOW(3), NOW(3), NULL),
(921002, 921002, 'dbcase_boundary_user2@penmate.local', '$2a$10$boundary', '冻结用户', 2, 'sso', NULL, NOW(3), NOW(3), NULL)
...
(921001, 921001, 920002, RPAD('边界项目', 200, '测'), NULL, 0, 1, NOW(3), NOW(3), NULL)
ON DUPLICATE KEY UPDATE updated_at = VALUES(updated_at);
...
(921001, 921001, 921001, '边界默认文风', 1, 'slow', 'cold', 'world', '保持克制叙述', '', NOW(3), NOW(3), NULL),
(921002, 921002, 921001, '边界实验文风', 0, 'fast', 'chaotic', 'plot', '允许高密度短句', NULL, NOW(3), NOW(3), NULL)
ON DUPLICATE KEY UPDATE updated_at = VALUES(updated_at);

-- Plugin：禁用安装与失败调用
INSERT INTO plugin_catalog (id, plugin_id, code, name, category, provider, status, latest_version, created_at, updated_at) VALUES
(921001, 921001, 'boundary-plugin', '边界插件', 'test', 'penmate', 'inactive', '0.0.1', NOW(3), NOW(3))
ON DUPLICATE KEY UPDATE updated_at = VALUES(updated_at);
...
(921001, 921001, 921001, 921001, '0.0.1', JSON_OBJECT('mode', 'safe'), 0, 920001, NOW(3), NOW(3))
ON DUPLICATE KEY UPDATE updated_at = VALUES(updated_at);
...
(921001, 921001, 921001, 'boundary-plugin', 'dry_run', JSON_OBJECT('input', 'x'), NULL, 9999, 'failed', RPAD('timeout', 120, 'x'), NOW(3))
...
(921001, 921001, 921001, 920002, '边界会话', JSON_OBJECT('scope', 'minimal'), NOW(3), 'active', NOW(3), NOW(3), NULL)
ON DUPLICATE KEY UPDATE updated_at = VALUES(updated_at);
...
(921001, 921001, 921001, 921001, 921001, 'rewrite', JSON_OBJECT('prompt', ''), JSON_OBJECT('styleId', 921001), JSON_ARRAY('boundary-plugin'), 'failed', NOW(3), NOW(3), 'empty prompt', NOW(3))
ON DUPLICATE KEY UPDATE created_at = VALUES(created_at);
...
(921001, 921001, 921001, 921001, 'rewrite', JSON_OBJECT('chapterId', 921001), 3, 'pending', 920002, NULL, NULL, NULL, NOW(3), NOW(3))
ON DUPLICATE KEY UPDATE updated_at = VALUES(updated_at);
...
(921001, 921001, 921001, 'manual', '边界文档', 'upload://boundary', 'dbcase/rag/921001/origin.txt', NULL, 'text/plain', 'failed', 'pending', NOW(3), NOW(3), NULL)
ON DUPLICATE KEY UPDATE updated_at = VALUES(updated_at);
...
(921001, 921001, 921001, 921001, 1, 'x', 1, 'vec-921001-1', 'milvus', 'openai', 'text-embedding-3-small', 1536, 'v1', JSON_OBJECT('boundary', true), NOW(3))
ON DUPLICATE KEY UPDATE created_at = VALUES(created_at);
...
(921001, 921001, 'migration', 'novel:921001', 'failed', 'conflict detected', NOW(3), NOW(3), NOW(3), NOW(3))
ON DUPLICATE KEY UPDATE updated_at = VALUES(updated_at);
...
(921001, 921001, 'content_to_object_storage', 'running', 1, JSON_OBJECT('migrated', 0, 'failed', 0), NULL, NOW(3), NULL, NOW(3), NOW(3))
ON DUPLICATE KEY UPDATE updated_at = VALUES(updated_at);
...
-- INSERT INTO iam_users (id,user_id,email,password_hash,display_name,status,auth_method,created_at,updated_at)
-- VALUES (921101,921101,'dbcase_admin@penmate.local','$2a$10$x','dup-email',1,'local',NOW(3),NOW(3));

-- CASE-CONFLICT-002: 章节版本唯一键冲突（novel_chapter_versions.uk_chapter_version_no）
-- INSERT INTO novel_chapter_versions (id,chapter_version_id,chapter_id,version_no,change_type,snapshot_object_key,created_by,created_at)
-- VALUES (921102,921102,920001,2,'rewrite','dup-version',920002,NOW(3));

-- CASE-CONFLICT-003: 插件安装唯一键冲突（plugin_project_installs.uk_project_plugin）
-- INSERT INTO plugin_project_installs (id,plugin_install_id,project_id,plugin_id,version,enabled,installed_by,installed_at,updated_at)
-- VALUES (921103,921103,920001,920001,'1.3.0',1,920002,NOW(3),NOW(3));
...
-- INSERT INTO rag_chunks (id,chunk_id,project_id,document_id,chunk_no,content_text,token_count,vector_id,vector_store,embedding_provider,embedding_model,embedding_dim,embedding_version,created_at)
-- VALUES (921104,921104,921001,921001,2,'dup vector',2,'vec-921001-1','milvus','openai','text-embedding-3-small',1536,'v1',NOW(3));

-- CASE-BOUNDARY-ILLEGAL-001: 非法状态注入（应用层应拒绝）
-- INSERT INTO agent_approval_requests (id,approval_request_id,project_id,task_id,approval_type,payload_json,risk_level,status,requested_by,created_at,updated_at)
-- VALUES (921105,921105,921001,921001,'rewrite',JSON_OBJECT('chapterId',921001),2,'UNKNOWN_STATUS',920002,NOW(3),NOW(3));
