...
class AgentCheckpointServiceTest {
    ...
    @Mock
    private AgentCheckpointCache checkpointCache;
    @Mock
    ...
    void continuation_artifact_event_is_a_checkpoint_boundary() {
        AgentCheckpointService service = new AgentCheckpointService(
                checkpointRepository, checkpointCache, incrementingIds(81000L),
                new JacksonJsonCodec(new ObjectMapper()), objectStorage);
    ...
    void falls_back_to_previous_checkpoint_when_latest_is_corrupt() throws Exception {
        ...
        AgentRuntimeState restored = service(mapper).loadLatest(70001L);
    ...
    private AgentCheckpointService service(ObjectMapper mapper) {
        return new AgentCheckpointService(
                checkpointRepository, checkpointCache, incrementingIds(81000L),
                new JacksonJsonCodec(mapper), objectStorage);
    }
