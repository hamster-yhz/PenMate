...
public interface AgentMapper {
    ...
    @Select("""
            SELECT id, project_id, user_id, title,
                   ...
                   last_message_at, status, created_at, updated_at, deleted_at
            FROM agent_conversations
            WHERE project_id = #{projectId} AND id = #{conversationId} AND deleted_at IS NULL
            LIMIT 1
            """)
    ...
    @Insert("""
            INSERT INTO agent_conversations(project_id, user_id, title, context_scope_json, status)
            VALUES (#{projectId}, #{userId}, #{title}, #{contextScopeJson}, #{status})
            """)
    @Options(useGeneratedKeys = true, keyProperty = "id")
    int insertConversation(AgentConversation conversation);

    @Select("""
            SELECT id, conversation_id, role, user_message_type, content_md,
    ...
    @Insert("""
            INSERT INTO agent_messages(conversation_id, role, user_message_type, content_md, attachments_json, tool_calls_json, seq_no)
            VALUES (#{conversationId}, #{role}, #{userMessageType}, #{contentMd}, #{attachmentsJson}, #{toolCallsJson}, #{seqNo})
            """)
    @Options(useGeneratedKeys = true, keyProperty = "id")
