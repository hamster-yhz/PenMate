  ...
  <section class="settings-surface model-services">
    ...
    <div v-else class="model-list" role="table" aria-label="个人模型列表">
      ...
      <div v-for="item in filteredConfigurations" :key="item.modelConfigId" class="model-row" role="row">
        <div class="model-identity"><strong>{{ item.displayName || item.modelName }}</strong><small>{{ item.modelName }}</small><small v-if="item.modelType === 'CHAT'">{{ capacityLabel(item) }}</small><small v-if="item.modelType === 'CHAT'">{{ reasoningLabel(item) }}</small></div>
