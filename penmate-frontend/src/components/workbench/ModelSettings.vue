<template>
  <div v-if="visible" class="model-settings">
    <button type="button" class="ms-backdrop" aria-label="关闭模型设置" @click="$emit('close')"></button>
    <div class="ms-modal glass-panel" role="dialog" aria-modal="true" aria-labelledby="model-settings-title">
      <div class="ms-header">
      ...
      <div class="ms-body">
          ...
          <div class="section-title-row">
            <div>
              ...
              <p class="section-desc">
                用户视角只维护模型配置；创建与编辑时直接填写 Key，模型绑定 key 仅作为后端内部实现。
              </p>
            </div>
          ...
          <div v-else class="config-list">
            ...
            >
              ...
              <div class="config-card-tags">
                <span v-if="mainAgentModelConfigId === String(item.modelConfigId)" class="config-tag primary"
                  >主 Agent</span
                >
                <span v-if="dirtyWorkAgentModelConfigId === String(item.modelConfigId)" class="config-tag secondary"
                  >Dirty Work Agent</span
                >
              </div>
        ...
        <section v-if="isFormVisible" class="api-form">
          ...
          <div class="form-grid">
            <div class="form-row">
              <label>供应商</label>
              <select v-model="form.providerId" class="f-input" aria-label="模型提供商">
...
const toModelCategory = (keySourceType: KeySourceType) =>
  keySourceType === 'OFFICIAL_KEY' ? 'OFFICIAL_MODEL' : 'USER_MODEL'
...
const loadData = async () => {
  ...
  try {
    ...
    const resolvedConfigs = normalizedConfigs.length > 0 ? normalizedConfigs : fallbackConfigs
    ...
    userConfigs.value = resolvedConfigs
      .map((item) => {
        ...
        return {
          ...
          providerId,
          providerName:
            providerOptions.value.find((provider) => provider.providerId === providerId)?.providerName ||
            `provider-${providerId}`,
    ...
    mainAgentModelConfigId.value = normalizeAssignedConfigId(
      normalizeBusinessStringId(preferenceRecord.mainAgentModelConfigId),
    )
    dirtyWorkAgentModelConfigId.value = normalizeAssignedConfigId(
      normalizeBusinessStringId(preferenceRecord.dirtyWorkAgentModelConfigId),
    )
...
const saveConfig = async () => {
  ...
  const requiresApiKey =
    !editingConfig || editingConfig.providerId !== form.providerId || editingConfig.keySourceType !== form.keySourceType
...
.config-card {
  text-align: left;
  background: linear-gradient(180deg, rgba(11, 17, 32, 0.62), rgba(11, 17, 32, 0.42));
  border: 1px solid var(--border-subtle);
  border-radius: 14px;
  padding: 14px;
  color: var(--text-primary);
...
.config-card.assigned {
  background: linear-gradient(180deg, rgba(35, 48, 74, 0.7), rgba(11, 17, 32, 0.42));
}
.config-card-head {
  display: flex;
  justify-content: space-between;
...
.config-type-badge.user {
  color: #9cc8ff;
  background: rgba(115, 181, 255, 0.14);
}
.config-type-badge.official {
  color: var(--amber-gold);
