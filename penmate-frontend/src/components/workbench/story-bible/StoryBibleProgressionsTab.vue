    ...
    <form class="progression-form" @submit.prevent="submit">
      <label>
        ...
        <select v-model="anchorChapterId" required>
          ...
          <option v-if="anchorChapterId && !hasChapter(anchorChapterId)" :value="anchorChapterId">{{ chapterLabel(anchorChapterId) }}</option>
          <option v-for="chapter in chapters" :key="chapter.chapterId" :value="chapter.chapterId">{{ chapterLabel(chapter.chapterId) }}</option>
        </select>
      </label>
      <label>
        <span>结束章节</span>
        <select v-model="endChapterId">
          ...
          <option v-if="endChapterId && !hasChapter(endChapterId)" :value="endChapterId">{{ chapterLabel(endChapterId) }}</option>
          <option v-for="chapter in chapters" :key="chapter.chapterId" :value="chapter.chapterId">{{ chapterLabel(chapter.chapterId) }}</option>
        </select>
      </label>
        ...
        <template v-if="editingProgressionId === item.progressionId">
          <div class="progression-edit">
            <select v-model="editAnchorChapterId" required aria-label="编辑起始章节">
              <option v-if="editAnchorChapterId && !hasChapter(editAnchorChapterId)" :value="editAnchorChapterId">{{ chapterLabel(editAnchorChapterId) }}</option>
              <option v-for="chapter in chapters" :key="chapter.chapterId" :value="chapter.chapterId">{{ chapterLabel(chapter.chapterId) }}</option>
            </select>
            <select v-model="editEndChapterId" aria-label="编辑结束章节">
              <option value="">持续生效</option>
              <option v-if="editEndChapterId && !hasChapter(editEndChapterId)" :value="editEndChapterId">{{ chapterLabel(editEndChapterId) }}</option>
              <option v-for="chapter in chapters" :key="chapter.chapterId" :value="chapter.chapterId">{{ chapterLabel(chapter.chapterId) }}</option>
            </select>
...
input, select, textarea, button { min-width: 0; border: 1px solid var(--border-subtle); border-radius: 4px; color: var(--text-primary); background: rgba(11, 17, 32, 0.7); }
...
.progression-row { min-height: 70px; display: grid; grid-template-columns: minmax(180px, 0.8fr) minmax(0, 1fr) 68px; align-items: center; gap: 12px; border-bottom: 1px solid var(--border-subtle); }
.anchor { display: flex; align-items: center; flex-wrap: wrap; gap: 5px; color: var(--amber-gold); font-size: 0.7rem; }
...
@media (max-width: 680px) {
  ...
  .progression-row { grid-template-columns: minmax(0, 1fr) 68px; align-items: start; padding: 10px 0; }
  .anchor, .progression-copy, .progression-edit { grid-column: 1; }
  .row-actions { grid-column: 2; grid-row: 1 / span 2; }
  .progression-edit { grid-template-columns: 1fr; }
  .progression-edit textarea { grid-column: 1; }
}
</style>
