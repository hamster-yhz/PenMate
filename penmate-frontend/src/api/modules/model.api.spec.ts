...
describe('model.api', () => {
  ...
  it('should_call_user_model_config_list_endpoint_when_list_user_model_configs_invoked', async () => {
    ...
    await modelApi.listUserModelConfigs('user-99')

    expect(getMock).toHaveBeenCalledWith('/v1/model/configs?userId=user-99')
  })

  it('should_reject_provider_responses_that_only_expose_legacy_physical_id', async () => {
    getMock.mockResolvedValue([
      { id: 1, displayName: 'OpenAI Legacy' },
    ])

    await expect(modelApi.listProviders()).rejects.toThrow('Invalid provider contract')
    expect(getMock).toHaveBeenCalledWith('/v1/model/providers')
  })

  it('should_keep_semantic_provider_id_without_using_id_as_fallback', async () => {
    getMock.mockResolvedValue([
  ...
  it('should_use_string_business_model_config_id_in_update_and_delete_endpoints_without_leaking_physical_or_duplicate_ids_in_payload', async () => {
    ...
    await modelApi.updateUserModelConfig('user-12', businessId, 'operator-7', {
      id: physicalId,
      modelConfigId: businessId,
      modelName: '  qwen-plus  ',
    })
    await modelApi.deleteUserModelConfig('user-12', businessId, 'operator-7')

    expect(putMock).toHaveBeenCalledWith('/v1/model/configs/mcfg-900719925474099312345?userId=user-12&operatorId=operator-7', {
      modelName: 'qwen-plus',
    })
    expect(putMock.mock.calls[0][1]).not.toHaveProperty('id')
    expect(putMock.mock.calls[0][1]).not.toHaveProperty('modelConfigId')
    expect(deleteMock).toHaveBeenCalledWith('/v1/model/configs/mcfg-900719925474099312345?userId=user-12&operatorId=operator-7')
  })
  ...
  it('should_call_user_model_config_create_endpoint_with_string_business_ids_and_direct_key_value_payload', async () => {
    ...
    await modelApi.createUserModelConfig('user-99', 'operator-7', {
    ...
    expect(postMock).toHaveBeenCalledWith('/v1/model/configs?userId=user-99&operatorId=operator-7', {
      providerId: 'provider-openai-900719925474099312345',
      modelName: 'gpt-4.1',
      baseUrl: 'https://api.example.com',
      modelCategory: 'USER_MODEL',
  ...
  it('should_call_user_model_config_update_endpoint_with_string_business_ids_and_direct_key_value_payload', async () => {
    ...
    await modelApi.updateUserModelConfig('user-12', 'mcfg-3', 'operator-7', {
    ...
    expect(putMock).toHaveBeenCalledWith('/v1/model/configs/mcfg-3?userId=user-12&operatorId=operator-7', {
      providerId: 'provider-qwen-3',
      modelName: 'qwen-max',
      baseUrl: undefined,
      modelCategory: 'OFFICIAL_MODEL',
  ...
  it('should_not_silently_drop_non_blank_string_provider_id_when_create_or_update_user_model_config', async () => {
    ...
    await modelApi.updateUserModelConfig('user-99', 'mcfg-9001', 'operator-7', {
    ...
    expect(putMock).toHaveBeenCalledWith('/v1/model/configs/mcfg-9001?userId=user-99&operatorId=operator-7', {
      providerId: 'provider-negative-like',
      modelName: 'gpt-4.1',
      modelCategory: 'OFFICIAL_MODEL',
      apiKey: 'sk-update',
  ...
  it('should_call_user_model_config_delete_endpoint_when_delete_user_model_config_invoked_with_string_business_id', async () => {
    ...
    await modelApi.deleteUserModelConfig('user-12', 'mcfg-3', 'operator-7')

    expect(deleteMock).toHaveBeenCalledWith('/v1/model/configs/mcfg-3?userId=user-12&operatorId=operator-7')
  })
  ...
  it('should_call_user_model_preferences_detail_endpoint_when_get_user_model_preferences_invoked', async () => {
    ...
    await modelApi.getUserModelPreferences('user-1001')

    expect(getMock).toHaveBeenCalledWith('/v1/model/preferences?userId=user-1001')
  })
  ...
  it('should_call_user_model_preferences_endpoint_with_string_assignment_fields_even_when_one_side_is_empty', async () => {
    ...
    await modelApi.saveUserModelPreferences('user-1001', 'operator-1001', {
      mainAgentModelConfigId: 'mcfg-900719925474099312345',
      dirtyWorkAgentModelConfigId: undefined,
      extraField: 'legacy',
    })

    expect(postMock).toHaveBeenCalledWith('/v1/model/preferences?userId=user-1001&operatorId=operator-1001', {
      mainAgentModelConfigId: 'mcfg-900719925474099312345',
  ...
  it('should_reject_number_business_ids_at_compile_time', () => {
    ...
    modelApi.listUserModelConfigs(99)
    // @ts-expect-error business IDs must be string-only
    modelApi.updateUserModelConfig('user-1', 'mcfg-1', 7, {})
    // @ts-expect-error business IDs must be string-only
    modelApi.saveUserModelPreferences(1001, 'operator-1001', {})
  })
})
